...
var stats = function(spec, my) {
  ...
  update_modules_state = function() {
    breach.module('core').call('modules_list', {}, function(err, modules) {
      modules.forEach(function(m) {
        if(m.running) {
          common.log.out(my.client_id + ': /modules/running/' + m.type + '/' + m.name + '/' + m.version);
        }
        else {
          common.log.out(my.client_id + ': /modules/stopped/' + m.type + '/' + m.name + '/' + m.version);
        }
      });
    });
  };
  ...
  core_inst_rpc_call_handler = function(rpc_call) {
    ...
    common.log.out(my.client_id + ': /rpc_call/' + rpc_call.dst + '/' + rpc_call.prc + '/' + rpc_call.src);
  };
